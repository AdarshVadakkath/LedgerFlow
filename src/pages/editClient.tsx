...
export function EditClient({ open, client, onClose }: EditClientProps) {
  ...
  // Re-sync form default values if client data changes
  useEffect(() => {
    reset({
      business_name: client.businessName,
      pan: client.pan || "",
  ...
  const onSubmit = (data: CreateClientInput) => {
    setServerError(null);
    mutate(
      { id: client.id, input: data },
      {
        onSuccess: () => {
          onClose();
        },
        onError: (error: any) => {
          let errorMsg = error?.message || "Failed to update client";
    ...
    >
      ...
      <Card className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <CardHeader>
          ...
          <CardDescription>
            Update the information for {client.businessName}
          </CardDescription>
        </CardHeader>
